...
SDValue SelectionDAGLegalize::ExpandBUILD_VECTOR(SDNode *Node) {
  ...
  if (SplatValue.getNode()) {   // Splat of one value?
    ...
    if (TLI.isShuffleMaskLegal(&ZeroVec[0], Node->getValueType(0))) {
      ...
      return DAG.getVectorShuffle(Node->getValueType(0), dl, LowValVec,
  ...
  if (Values.size() == 2) {
    ...
    if (TLI.isOperationLegalOrCustom(ISD::SCALAR_TO_VECTOR,
                                     Node->getValueType(0)) &&
        TLI.isShuffleMaskLegal(&ShuffleMask[0], Node->getValueType(0))) {
      ...
      return DAG.getVectorShuffle(Node->getValueType(0), dl, Val1, Val2,
                                  &ShuffleMask[0]);
    }
  }
